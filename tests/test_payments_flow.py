...
from nexolu_payments_core.core.memory.entities import Merchant
...
async def test_provisioning_creates_merchant(client, monkeypatch):
    ...
    response = await client.post(
        "/v1/payments/v1/admin/merchants",
...
async def test_provisioning_requires_server_key(client):
    response = await client.post("/v1/payments/v1/admin/merchants", json={"name": "x", "slug": "x"})
    assert response.status_code == 401
