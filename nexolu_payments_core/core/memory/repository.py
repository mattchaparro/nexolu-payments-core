"""Persistence queries shared by auth, admin and payment services."""
...
async def get_active_credential(
    session: AsyncSession, merchant_id: str, provider_slug: str, environment: str = "sandbox"
...
async def get_active_fee_schedule(
    session: AsyncSession, merchant_id: str, provider_slug: str
) -> FeeSchedule | None:
    stmt = (
        select(FeeSchedule)
        .where(
